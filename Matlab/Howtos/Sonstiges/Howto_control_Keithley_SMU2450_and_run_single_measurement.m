...
% use SMU as power supply unit (current source with maximum voltage limit)
mySMU.OperationMode  = 'SIMV';
mySMU.SourceParameters.OutputValue   = 20e-3; % 20 mA
mySMU.SourceParameters.LimitValue    = 2.5;   % limit to 2.5 V
...
%mySMU.SenseParameters.NPLCycles          = 1; % default: 1
%mySMU.SenseParameters.AverageCount       = 0; % default: 0 = no filtering

% optionally enable output before actual measurement
%mySMU.outputEnable;
...
result = mySMU.runMeasurement( ...
    timeout   =  60      , ...
    mode      = 'simple' , ...
    count     = 300      );    % 100 measurement values (over time)

% show resulting V(t) diagram
figure(1);
plot(result.timestamps, result.senseValues , '-b*');
