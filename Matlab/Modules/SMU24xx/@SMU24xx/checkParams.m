...
for nArgsIn = 2:2:length(inVars)
    ...
    if ischar(paramName) || isStringScalar(paramName)
        ...
        elseif isa(paramValue, 'double') || islogical(paramValue)
            if length(paramValue) > 2500
                % no input parameter is longer than 2500 elements ('list')
                paramValue = paramValue(1:2500);
                disp(['SMU24xx: Length of ''' paramName ''' exceeds ' ...
                'limit of 2500 elements. Ignore remaining elements.']);
            end
