classdef SMUMacros < handle
    ...
    properties(Constant = true)
        MacrosVersion = '1.0.4';      % Updated release version
        MacrosDate    = '2025-05-06'; % Updated release date
    end

    properties(Dependent, SetAccess = private, GetAccess = public)
        OutputState        % Current output state ('on' or 'off')
        ShowMessages       % Logical indicating if messages are displayed
    end

    properties(SetAccess = private, GetAccess = private)
        SMUObj            % Reference to SMU object for communication
        DeviceID          % Device identifier (e.g., 'KEITHLEY-2450')
        OutputStateValue  % Stored output state ('on' or 'off')
    end
%% 

    methods
        function obj = SMUMacros(smuObj)
            % Constructor
            % Input: smuObj - SMU object instance
            obj.SMUObj = smuObj;
            obj.OutputStateValue = 'off'; % Initialize output state
            
            % Get device identifier with proper error handling
            try
                if isprop(smuObj, 'DeviceName')
                    obj.DeviceID = smuObj.DeviceName;
                elseif isprop(smuObj, 'Name')
                    obj.DeviceID = smuObj.Name;
                else
                    obj.DeviceID = 'KEITHLEY-2450';
                end
            catch
                obj.DeviceID = 'KEITHLEY-2450';
            end
            
            % Display initialization message only if ShowMessages is enabled
            try
                if ~strcmpi(obj.SMUObj.ShowMessages, 'none')
                    disp(['SMUMacros: Initialized for ' obj.DeviceID]);
                end
            catch
                % Silent fallback if message display fails
            end
        end
%% 
        ...
        function delete(obj)
            ...
            try
                if ~strcmpi(obj.SMUObj.ShowMessages, 'none')
                    disp(['SMUMacros: Closing connection to ' obj.DeviceID]);
                end
            catch
                % Silent fallback
            end
...
%% 
        ...
        function status = runAfterOpen(obj)
            % Execute post-open configuration
            % Initialize Keithley 2450
            status = -1; % Default to error state
            
            try
                if ~strcmpi(obj.SMUObj.ShowMessages, 'none')
                    disp([obj.DeviceID ': Executing post-open configuration']);
                end
                
                % Reset device with proper command sequence
                 obj.SMUObj.write('*RST');
                 obj.SMUObj.opc; % Wait for operation complete
                
                % Configure default settings
                %obj.SMUObj.write(':SYST:BEEP:STAT OFF'); % Disable beeper
                obj.SMUObj.write('OUTP OFF');      % Ensure output is off
                obj.OutputStateValue = 'off';
                
                status = 0; % Success
            catch ME
                if ~strcmpi(obj.SMUObj.ShowMessages, 'none')
                    disp(['Error in runAfterOpen: ' ME.message]);
                end
            end
        end
...
%% 
        ...
        function status = runBeforeClose(obj)
            % Execute pre-close configuration
            % Ensure safe shutdown
            status = -1; % Default to error state
            
            try
                if ~strcmpi(obj.SMUObj.ShowMessages, 'none')
                    disp([obj.DeviceID ': Executing pre-close configuration']);
                end
                
                % Turn off output and wait for completion
                obj.SMUObj.write('OUTP OFF');
                obj.OutputStateValue = 'off';
                obj.SMUObj.opc;
                ...
                if ~strcmpi(obj.SMUObj.ShowMessages, 'none')
                    disp(['Error in runBeforeClose: ' ME.message]);
                end
            end
        end
%% 
        ...
        function status = reset(obj)
            ...
            try
                if ~strcmpi(obj.SMUObj.ShowMessages, 'none')
                    disp([obj.DeviceID ': Resetting device']);
                end
                ...
                if ~strcmpi(obj.SMUObj.ShowMessages, 'none')
                    disp(['Error in reset: ' ME.message]);
                end
            end
        end
%% 
        ...
        function status = clear(obj)
            ...
            try
                if ~strcmpi(obj.SMUObj.ShowMessages, 'none')
                    disp([obj.DeviceID ': Clearing status']);
                end
                
                obj.SMUObj.write('*CLS');
                obj.SMUObj.opc;
                
                status = 0; % Success
            catch ME
                if ~strcmpi(obj.SMUObj.ShowMessages, 'none')
                    disp(['Error in clear: ' ME.message]);
                end
            end
        end
        %% 
        ...
        function status = outputEnable(obj)
            ...
            try
                if ~strcmpi(obj.SMUObj.ShowMessages, 'none')
                    disp([obj.DeviceID ': Enabling output']);
                end
                
                obj.SMUObj.write(':OUTP ON');
                obj.OutputStateValue = 'on';
                obj.SMUObj.opc;
                
                status = 0; % Success
            catch ME
                if ~strcmpi(obj.SMUObj.ShowMessages, 'none')
                    disp(['Error in outputEnable: ' ME.message]);
                end
            end
        end
        %% 
        ...
        function status = outputDisable(obj)
            ...
            try
                if ~strcmpi(obj.SMUObj.ShowMessages, 'none')
                    disp([obj.DeviceID ': Disabling output']);
                end
                ...
                if ~strcmpi(obj.SMUObj.ShowMessages, 'none')
                    disp(['Error in outputDisable: ' ME.message]);
                end
            end
        end
        %% 
        ...
        %Tested, worked, 
        %mySMU.configureSenseMode('function', 'current', 'mode', '4WIRE');
...
function meas = measure(obj, varargin)
    ...
    try
        ...
        cmd = 'SENS:FUNC?';
        disp(['  Sending to SMU: ' cmd]);
        senseFunc = obj.SMUObj.query(cmd);
        ...
        elseif contains(senseFunc, '"VOLT') && strcmp(requestedFunc, 'POWER')
            ...
            if ischar(cmd)
                ...
                cmd = 'OPC?';
                disp(['  Sending to SMU: ' cmd]);
                opcResponse = obj.SMUObj.query(cmd);
        ...
        if meas.status == 0 && ~strcmpi(obj.SMUObj.ShowMessages, 'none')
   ...
   function [voltages, currents] = VoltageLinearSweep(obj, start, stop, numPoints, delay) %limit
    ...
    try
        if ~strcmpi(obj.SMUObj.ShowMessages, 'none')
            disp([obj.DeviceID ': Starting Voltage Linear Sweep']);
        end

        % Increase timeout
        obj.SMUObj.Timeout = 30; % Set to 30 seconds
        ...
        obj.SMUObj.write('INIT');
        if ~strcmpi(obj.SMUObj.ShowMessages, 'none')
            disp('  Sending to SMU: INIT');
        end
        
        obj.SMUObj.write('*WAI');
        if ~strcmpi(obj.SMUObj.ShowMessages, 'none')
        ...
        obj.SMUObj.write('OUTP OFF');
        if ~strcmpi(obj.SMUObj.ShowMessages, 'none')
            disp('  Sending to SMU: OUTP OFF');
            disp([obj.DeviceID ': Linear Sweep completed']);
        end
    ...
    try
        if ~strcmpi(obj.SMUObj.ShowMessages, 'none')
            disp([obj.DeviceID ': Starting Current Linear Sweep']);
        end
        ...
        % Configure sweep
        obj.SMUObj.write(sprintf('SOUR:SWE:CURR:LIN:STEP %f, %f, %f, %f, 1, AUTO', start, stop, step, delay));
        if ~strcmpi(obj.SMUObj.ShowMessages, 'none')
    ...
    methods
        function outputState = get.OutputState(obj)
            ...
            try
                response = obj.SMUObj.query(':SOUR:OUTP?');
                if contains(response, '1')
                    obj.OutputStateValue = 'on';
                else
                    obj.OutputStateValue = 'off';
                end
                outputState = obj.OutputStateValue;
                
                if ~strcmpi(obj.SMUObj.ShowMessages, 'none')
                    disp([obj.DeviceID ': Output state = ' outputState]);
                end
            catch ME
                error('SMUMacros: Failed to get OutputState: %s', ME.message);
            end
        end

        function set.OutputState(obj, value)
            % Set output state ('on' or 'off')
            if ~ismember(lower(value), {'on', 'off'})
                error('SMUMacros: OutputState must be ''on'' or ''off''');
            end
            obj.OutputStateValue = lower(value);
        end

        function showmsg = get.ShowMessages(obj)
            % Get ShowMessages property from SMU object
            try
                if isprop(obj.SMUObj, 'ShowMessages')
                    showmsg = ~strcmpi(obj.SMUObj.ShowMessages, 'none');
                else
                    showmsg = false;
                end
            catch
