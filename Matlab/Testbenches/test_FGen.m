...
myFGen.arbWaveform(          ...
    'mode'    , 'list'     , ...
    'submode' , 'all'      );
[out1, out2] = myFGen.arbWaveform('mode', 'list', 'submode', 'vol')
...
myFGen.arbWaveform(          ...
    'mode'    , 'upload'   , ...
    'waveData', abs(sin(2*pi*(0:999)/1000)+0.5)-0.75, ...
    'waveName', 'dummy4' );
...
% remove all "
resultList = strrep(char(response), '"', '');
resultList = split(char(resultList), ',');
% check number of elements
numFiles = (length(resultList)-2) /3;
if (numFiles < 0) || ceil(numFiles) ~= floor(numFiles)
    status = -1
    % something went wrong ==> break
else
    memUsed  = resultList{1}
    memFree  = resultList{2}
    memFiles = reshape(resultList(3:end), 3, numFiles)'
    numFiles
end
