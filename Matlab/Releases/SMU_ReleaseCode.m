...
ModuleName = 'SMU';  % name of class file
%VersionID  = '3.0.0_2025-0x-xx'; % should match name of tag in git (version control)
VersionID  = 'x.y.z';
...
mhelp   = help(fullfile(workSourceDir, 'SMU.m'));
fid     = fopen(fullfile(workReleaseDir, 'SMU.m'), 'w');
fwrite(fid,['%' strrep(mhelp, newline, sprintf('\n%%'))]);
fclose(fid);

% create p-files out of original m-files
pcode(fullfile(workSourceDir, 'SMU.m'));
pcode(fullfile(workSourceDir, 'checkParams.m'));
pcode(fullfile(workSourceDir, 'listAvailablePackages.m'));

% -------------------------------------------------------------------------
workSourceDir  = fullfile(SourceDir,  PackageDirName, '+Keithley', '+Model2450');
workReleaseDir = fullfile(ReleaseDir, PackageDirName, '+Keithley', '+Model2450');
mkdir(workReleaseDir);
cd(workReleaseDir);

% create p-files out of original m-files
pcode(fullfile(workSourceDir, 'SMUMacros.m'));
